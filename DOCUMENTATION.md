...
### [2026-08-04]: Metadati fastlane per la 2.0 — 11 localizzazioni App Store
* **Dettagli**: Generato l'intero set di metadati `deliver` per la release 2.0 in 11 lingue, partendo da una sola localizzazione (`en-GB`) che descriveva ancora la 1.x. La descrizione precedente era ferma a 3.867 caratteri su 4.000 e non nominava nessuna delle funzioni nuove (doppio, rosa, storico, statistiche, iCloud, widget, Live Activity interattiva, export), quindi è stata riscritta anziché estesa: non c'era spazio per aggiungere.
* **Tech Notes**:
  - **Locale coperti (11)**: `en-US` (mancava del tutto), `en-GB`, `it`, `de-DE`, `fr-FR`, `es-ES`, `es-MX`, `pt-BR`, `ja`, `ko`, `zh-Hans`. Ogni cartella contiene 10 file: `name`, `subtitle`, `description`, `keywords`, `promotional_text`, `release_notes`, `marketing_url`, `privacy_url`, `support_url`, `apple_tv_privacy_policy`.
  - **Nome invariato**: `name.txt` è `Ping Pong Counter` byte per byte in tutti gli 11 locale, per scelta esplicita.
  - **Strategia ASO**: Apple indicizza l'**unione** di `name` + `subtitle` + `keywords`. Poiché il nome resta inglese ovunque, nei 9 locale non inglesi l'intera superficie di ricerca in lingua locale è `subtitle` (30) + `keywords` (100) = 130 caratteri. Il sottotitolo è quindi trattato come un secondo campo keyword, non come una traduzione: nessuna parola si ripete tra i tre campi. La configurazione `en-GB` precedente sprecava ~46 dei 100 caratteri di keyword in termini già indicizzati da nome e sottotitolo (`ping`, `pong`, `table`, `tennis`, `score`, `counter`, `tracker`).
  - **Descrizione**: struttura "gancio + blocchi puntati", ~1.800-2.300 caratteri contro i 3.867 precedenti. La descrizione **non** è indicizzata da Apple (a differenza di Google Play), quindi è copy di conversione, non ASO; più corta significa anche 11 file più economici da mantenere. Le funzioni 2.0 stanno in alto ma **non** sotto un'etichetta "NEW IN 2.0", che invecchia alla 2.1: quel lavoro lo fa `release_notes.txt`.
  - **Avviso lingua interfaccia**: l'app è localizzata solo in inglese e italiano (`PingPong/Localized.swift` è uno switch a runtime su `isItalian`). Le 8 descrizioni in lingue non supportate chiudono con una riga esplicita ("l'interfaccia è in inglese"); `en-US`, `en-GB` e `it` non la contengono.
  - **iPad**: `TARGETED_DEVICE_FAMILY = "1,2"` — l'app viene distribuita anche su iPad, e la descrizione precedente non lo nominava mai. Ora la prima riga dice "iPhone, iPad and Apple Watch" in tutti i locale.
  - **Sport adiacenti esclusi di proposito**: `validTargetScoreRange = 1...99` con `winByTwo` renderebbe l'app usabile per badminton (21), pallavolo (25), pickleball e squash (11), ma la rotazione del servizio è fissa a 2 o 5 battute, cioè specifica ITTF. Rivendicare quegli sport nelle keyword è un rischio 2.3.7 (funzione dichiarata e non presente) per un traffico che rimbalzerebbe comunque. Sbloccabile in una 2.1 aggiungendo un'opzione "servizio a chi vince il punto".
  - **Categorie invertite**: `primary_category` da `UTILITIES` a `SPORTS`, `secondary_category` da `SPORTS` a `UTILITIES`. UTILITIES è fra le categorie più affollate dello Store e per un segnapunti non è classificabile; SPORTS è molto più sottile e le classifiche per paese sono raggiungibili. I quattro file `*_sub_category.txt` restano vuoti: App Store Connect offre sottocategorie solo per Giochi e Sticker.
  - **Copyright**: da `© Mattioli Simone` a `2026 Simone Mattioli`. Apple aggiunge da sé il glifo ©, quindi il valore precedente lo stampava due volte e non riportava l'anno.
...
### [2026-08-04]: Rifacimento completo del sito GitHub Pages
* **Dettagli**: Il sito era una SPA in JavaScript con tre "tab" (`.view-section`), di cui solo una visibile alla volta, e con **nessun link all'App Store da nessuna parte** (la CTA primaria dell'hero era "Contact Support"). In più era rotto in produzione: 16 dei 23 blocchi `.reveal` restavano a `opacity: 0` perché l'IntersectionObserver non li raggiungeva, quindi la maggior parte della pagina non compariva mai. Riscritto da zero: `index.html`, `styles.css`, `main.js` nella **radice** del repo, con `docs/index.html` degradato a redirect. Contenuti allineati alla 2.0 (doppio, rosa, storico, statistiche, iCloud, widget, Live Activity, export, VoiceOver), che sul sito precedente non comparivano affatto.
* **Tech Notes**:
  - **Elemento distintivo — il tabellone giocabile**: l'hero non è uno screenshot ma un segnapunti funzionante. `main.js` porta in JS l'algoritmo reale: `serveTurns()` è la trascrizione di `DoublesLineup.serveTurns` (arrotondamento per eccesso dei turni prima dei vantaggi incluso), `deucePointTotal = 2 × (target − 1)`, il server deriva da `turni % 2` rispetto a `startOfSet`, `isSetWon` applica `winByTwo`, e a fine set il primo battitore si alterna come da ITTF. Controlli reali per punti (11/21), set (1/3/5), rotazione (2/5) e vantaggi. Verificato in browser: a 10-10 compare "Deuce" e serve il giocatore 1, a 11-10 "Set point" e serve il 2, a 12-10 il set va al giocatore 1 con reset e battuta invertita, l'undo riporta a 11-10 azzerando il pallino del set. Con vantaggi off a 10-10 si accendono **entrambe** le metà, come nell'app.
  - **Perché la radice e non `/docs`**: `PingPong/SettingsView.swift:23-24` puntava già a `.../Ping-Pong-Counter/#support` e `#privacy`, cioè alla radice, dove però c'era solo il redirect verso `docs/index.html` — che **perdeva il frammento**. Ora la radice è il sito vero e i link in-app funzionano. `docs/index.html` resta come redirect che **conserva l'hash** (`location.replace('../' + hash)`), perché la 1.0.2 pubblicata sullo Store punta ancora a `docs/index.html#privacy`.
  - **`scroll-behavior: smooth` rimosso**: su una pagina di ~12.000px animava il salto dall'hero alla privacy policy. Peggio, `location.replace()` non fa scrollare al frammento in modo affidabile: aggiunto in `main.js` un fallback che porta al target se dopo il caricamento si è ancora in cima. Verificato: `docs/index.html#privacy` → `/#privacy` a scrollY 10746, con la sezione a 99px dal bordo (barra sticky 63px).
  - **Design**: palette presa da `AppTheme.swift` — i tre temi dell'app (`#FF4059`/`#00B3FF`, `#00D98C`/`#8C4DE6`, `#FF8C00`/`#00CCCC`) sono commutabili sulla pagina e ritingono tutto. Motivo strutturale: la **rete tratteggiata**, lo stesso divisorio che l'app disegna tra le due metà, riusata come separatore di sezione. Tipografia: Archivo variabile (asse `wdth` 86-118) **self-hosted** — una pagina che dichiara "0 tracker" non può chiamare Google Fonts — più `ui-rounded` per i punteggi, che su dispositivi Apple rende in SF Pro Rounded, cioè il font vero dell'app.
  - **Peso**: `docs/assets/` erano 28 MB di PNG (uno da 2,6 MB). Gli asset serviti ora sono 371 KB in tutto: 6 WebP ritagliati sotto la didascalia marketing (i bordi sfumano via `mask-image`, così il taglio non si vede sul fondo scuro), icone, font e badge. OG image 1200×630 generata su misura (tabellone a 10-10 con badge DEUCE).
  - **SEO/GEO**: title 52 caratteri, meta description 154, canonical, OG e Twitter con URL assoluti, JSON-LD `@graph` (SoftwareApplication+MobileApplication, Person, WebSite, WebPage, FAQPage) con `@id` incrociati. **Nessun `aggregateRating`**: la scheda App Store non ha ancora abbastanza voti e inventarlo è una violazione delle policy sui dati strutturati. Gli H2 sono le domande di ricerca ("How do you keep score in table tennis?", "Can an Apple Watch keep the ping pong score?"). Nuovi `robots.txt` (crawler AI in allowlist esplicita), `sitemap.xml`, `llms.txt`, `llms-full.txt`. FAQ e JSON-LD generati e verificati byte per byte dallo stesso sorgente (12 domande, script di controllo).
  - **URL metadata aggiornati** in tutti i 12 locale: `marketing_url` alla radice, `support_url` e `privacy_url` con i rispettivi frammenti.
  - **Privacy policy riscritta**: la precedente diceva "we do not transmit" e "100% locally", affermazioni diventate false con la sincronizzazione iCloud della 2.0. Ora c'è una sezione dedicata che dice esattamente cosa viene copiato (storico e rosa, non il punteggio in corso), sotto quale Apple ID, e che lo sviluppatore non vi ha accesso. Aggiunta anche una sezione sul sito stesso (nessun cookie, nessun analytics, localStorage solo per il tabellone di prova).
  - **Verifica**: 5 lenti di review in parallelo (fatti vs sorgente Swift, accessibilità, SEO/GEO, CSS, JS), 64 finding grezzi, **59 confutati, 5 confermati** e corretti — vedi la voce successiva.

### [2026-08-04]: Correzioni dalla review del sito (5 finding confermati su 64)
* **Dettagli**: Ogni finding è passato per un agente incaricato di **confutarlo**; sono sopravvissuti solo cinque. Due critici, due sul focus da tastiera, uno di contrasto. Trovato inoltre in autonomia uno scroll orizzontale che la review aveva archiviato come cosmetico.
* **Tech Notes**:
  - **CRITICO — "Spoken umpire, in English and Italian" non è vero sulle build spedite**. L'italiano è tutto dietro `Localized.isItalian`, che legge `Locale.current.language.languageCode`. Ma il progetto dichiara solo l'inglese (`project.pbxproj:341-346`: `developmentRegion = en`, `knownRegions = (en, Base)`) e nel repo non esiste **nessun** `.lproj`/`.strings`/`.xcstrings`. In un bundle localizzato solo in inglese `Locale.current` si risolve sulla lingua **dell'app**, non del dispositivo: il verificatore ha compilato quattro bundle di prova e misurato `Locale.current.identifier == "en_IT"` su dispositivo italiano con zero `.lproj`, contro `it_IT` non appena si aggiunge `it.lproj` **oppure** `CFBundleLocalizations = (en, it)`. Quindi su un dispositivo italiano l'interfaccia e la voce restano in inglese. Rimossa la rivendicazione da `index.html`, dal JSON-LD (`inLanguage` ora `"en"`) e da entrambi i file `llms`. **La correzione lato app è in `TO_SIMO_DO.md`**: è un bug reale, non solo un problema di copy.
  - **CRITICO — il tabellone si tagliava in orizzontale**. `.board` aveva `aspect-ratio: 20/27` e `max-height` ma nessun tetto in larghezza: a 844×390 il rapporto diventava 1.538 invece di 0.741 e ogni metà aveva 45px di overflow, cancellati da `overflow: hidden`. Sparivano pallini dei set, nomi e badge SERVE — cioè esattamente ciò che il testo intorno pubblicizza. Ora i due assi sono vincolati insieme (`max-width: min(420px, calc(min(72vh, 560px) * 20 / 27))`), i contenuti si dimensionano sul **contenitore** e non sul viewport (`container-type: size` con unità `cqh`, dietro `@supports`), e `overflow: hidden` è stato tolto da `.side` perché `.board` già ritaglia. Misurato a 844×390: board 208×281, rapporto 0.741, overflow 0 su entrambe le metà, tutti gli elementi dentro il box.
  - **Scroll orizzontale di 84px** (trovato oltre la review, che lo aveva giudicato solo igiene): `.hero::before` usava `inset: -20% -10% auto -10%` e `body { overflow-x: hidden }` **non** lo conteneva — `window.scrollTo(200,0)` portava davvero `scrollX` a 84.5. Ora l'alone resta dentro il viewport. Verificato a 320, 390, 768, 844×390 e 1440: `canScrollX` 0 ovunque.
  - **Focus da tastiera invisibile in due punti**: l'anello globale ha `outline-offset: 3px`, ma `.board` ha `overflow: hidden` e `.topnav` ha `overflow-x: auto` (che forza `overflow-y: auto`), quindi entrambi lo ritagliavano — sulle due metà del tabellone, cioè i controlli principali della pagina, restava solo una riga che sembrava un divisorio. Aggiunti anelli interni (`outline-offset: -6px` sulle metà, `-2px` sui link di nav).
  - **Contrasto sotto AA sulla metà che serve**: `.who` usava `--ink-mute` (#79838f, 5.21:1 sul fondo pagina) ma sopra il tabellone c'è il tint del giocatore al 13%, che lo faceva scendere a 3.86:1 nel tema Mint. Passato a `--ink-dim` (#a6b0bb), che regge su tutti e tre i temi e su entrambe le metà.
  - **Confutati fra gli altri**: focus perso all'undo, mancanza di `<noscript>`, `<main>` non focalizzabile, `backdrop-filter` senza prefisso, `--gap` inutilizzato, ordine delle colonne in `.split-flip`. In ogni caso il verificatore ha misurato in browser prima di respingere.
